...
col_labels = ['Geometry (NA${}_{\\textrm{ill}}$ = 0.2, NA${}_{\\textrm{det}}$ = 0.8\n 3 detection polarizations)', 'Uncertainty Ellipse', r'$\sigma_{\Omega}$ [sr]', 'Median$\{\sigma_{\Omega}\}$ [sr]', 'MAD$\{\sigma_{\Omega}\}$ [sr]']
...
def calc_stats(param):
    ...
    exp = multiframe.MultiFrameMicroscope(ill_thetas=[np.pi/4, -np.pi/4], det_thetas=[-np.pi/4, np.pi/4],
                                          ...
                                          colors=['(1,0,0)', '(0,0,1)'], n_frames=1, n_det_frames=n_pol, 
...
def plot_2d_regions(ax, cax, pts, data, special_pt=(-1,-1)):
    ...
    my_annotate(ax, '\# of detection polarizations', (0.5, -0.13), fontsize=14)
...
fig.savefig('det-polarization-ortho.pdf', dpi=250)
