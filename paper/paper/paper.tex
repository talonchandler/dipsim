...
class. The single-view K\"ohler microscope with a small illumination NA and high
detection NA has the lowest solid-angle uncertainty median but the largest max
and fourfold symmetry. The dual-view oblique symmetric K\"ohler with an
...
\begin{tabular}{lllll}
  ...
  Microscope Type&$n$-fold&Max\{$\sigma_{\Omega}$\}&Median\{$\sigma_{\Omega}$\}&MAD\{$\sigma_{\Omega}$\}\\ 
    &Degeneracy&&&\\\midrule
Single-view K\"ohler (NA${}_\textrm{ill}$=0, NA${}_\textrm{det}$=1.1)&4&3.04$\times 10^{0}$&8.20$\times 10^{-4}$&1.48$\times 10^{-4}$ \\ 
  Dual-view oblique symmetric K\"ohler (NA=0.6, $\beta$=53${}^{\circ}$)&2&1.35$\times 10^{-1}$&4.49$\times 10^{-3}$&9.17$\times 10^{-4}$\\
